...
/** VARIED-probe contains: 1024 scrambled lookups (~50% hit / 50% miss) per invocation. Unlike the single-probe ContainsHit/Miss, the branch predictor can't
  * memorize the lookup — this exposes the real branch/ctrl-scan behavior (branchy byte-at-a-time probing collapses at high probe variety; branch-free holds).
  */
