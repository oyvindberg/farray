...
/** Symmetric difference (xor). Lazy: `(a ^ b).contains(hit)` — FSet distributes; competitors must build the symmetric diff. Materialized:
  * `(a ^ b).materialize.size`. scala has no native set xor → (a∖b) ∪ (b∖a).
  */
