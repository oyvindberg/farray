...
/** Persistent single-element update: produce a NEW set with one more (incl) / one fewer (excl) element. FSet's `+`/`-` are O(1) lazy nodes; scala immutable is
  * CHAMP path-copy; immutable BitSet copies the word array; mutable sets clone-then-mutate (the immutable-result cost).
  */
