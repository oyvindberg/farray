...
/** Persistent single-element update for String: a NEW set with one more (incl) / one fewer (excl) element. FSet's `+`/`-` are O(1) lazy; scala immutable is
  * CHAMP; mutable/Guava clone-or-rebuild (immutable-result cost).
  */
