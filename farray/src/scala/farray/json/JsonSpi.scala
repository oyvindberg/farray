...
/** The boundary between the fusion engine and the JSON byte decoder, expressed only in `q.reflect` terms and
  * functions over them — so the decoder never names the engine's `Shape`/`Tup`/`Ctx`/`Stage`, and the engine never
  * names the decoder's byte mechanics. One `Quotes` is threaded through (`q.type`) so every Expr/Term/callback lives
  * at the same staging level.
  *
  * Direction of flow:
  *   - engine → decoder: everything in [[JsonLowerInput]] (the record type, the runtime source, the pipeline's
  *     field-reading lambdas as plain Terms, and a `continue` callback the engine owns).
  *   - decoder → engine: per record, a [[JsonRecordColumns]] handed back THROUGH `continue` — the decomposed column
  *     reads, which the engine assembles into its own `Shape` and rejoins to the shared optimizer.
  */
final class JsonLowerInput[Q <: Quotes & Singleton](using val q: Q)(
    ...
    /** terminal readers whose parameter IS the record type (so they contribute live fields), pre-decomposed to
      * `(param, body)` by the engine (e.g. a fold op's element param, an `Agg.sum(_.x)` lambda) — JSON just walks them.
      */
    ...
    val continue: JsonRecordColumns[Q] => Expr[Unit]
)
...
/** what the decoder hands the engine per record: one column read per record field (live or dead), in declaration
  ...
  */
final class JsonRecordColumns[Q <: Quotes & Singleton](using val q: Q)(
    val columns: List[JsonColumn[Q]]
)
...
/** a single field's read, as a `Shape`-free CPS thunk: `read(k)` yields the field-value `Term` to `k` (so a lazy
  ...
  */
final class JsonColumn[Q <: Quotes & Singleton](using val q: Q)(
