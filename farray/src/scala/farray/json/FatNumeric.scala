...
/** The "many numeric fields, read few" demonstration (the user's hypothesis): when a record has MANY numeric fields and a query reads only one or two, the
  * lazy-decode win widens — because jsoniter (even with a narrow schema) must DECODE every numeric field its schema declares, paying the double-parse cost per
  * field per record, while the fused scanner decodes only the predicate field and skips the rest as raw bytes.
  *
...
object FatScan:
  ...
  /** QUERY: parseJson[FatRec].filter(_.m0 > t).map(_.m5).sum Decode m0 (predicate) and m5 (projection) ONLY; skip the other 14 numeric fields as raw bytes —
    * never parse them to doubles. jsoniter-narrow with schema {m0, m5} would also skip the other 14, so the FAIR comparison is against that. The win here is:
    * we decode exactly 2 doubles/record (and m5 only for survivors — the sink), jsoniter-narrow decodes 2 doubles AND allocates a Narrow case class/record.
    */
      ...
      while open do
        if buf(p) == '}' then open = false
          ...
          if !m0Seen && keyEquals(buf, ks, ke, FatKeys.m0) then
            ...
            p = ve
          else p = skipValue(buf, p, lineEnd) // the other 14 numerics: skipped as BYTES, never parsed
  ...
  /** QUERY (even more extreme): sum SEVERAL metrics for survivors only. parseJson[FatRec].filter(_.m0 > t).map(r => r.m1 + r.m2 + r.m3).sum m0 decodes always
    * (predicate); m1/m2/m3 decode ONLY for survivors (compute-for-survivors). The other 12 metrics are never decoded. jsoniter-narrow {m0,m1,m2,m3} decodes all
    * 4 for EVERY record. With low selectivity this is the multiplier: (#extra decoded) x (1 - selectivity).
    */
