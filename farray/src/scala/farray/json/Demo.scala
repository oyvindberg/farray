...
/** The v0a demonstration: a flat 20-field record, and the hand-written projection scanners that are EXACTLY the shape the `fuse` JSON macro will generate.
  * These prove the thesis before the macro is built — if these don't beat jsoniter-narrow on a selective query (with near-zero allocation on the aggregate), no
  * macro elegance saves it (docs/json-parser-research.md §5.6, the kill-or-prove criterion).
  *
  * Record shape (20 fields): a mix of longs / ints / doubles / strings, names of varied length so the length-then-first-byte-then-memcmp dispatch is exercised.
  */
...
object RecScan:
  ...
  /** QUERY A (aggregate, ZERO per-record allocation): parseJson[Rec].filter(_.amount > t).map(_.amount).sum Reads ONLY `amount` (1 of 20 fields), skips the
    * other 19 by the byte, decodes no strings, allocates no Rec, folds into a `var`. This is the constant-memory headline.
    *
    * Returns the sum of `amount` over records whose amount > threshold. `lineStart`..`lineEnd` is one NDJSON record (the streaming driver provides these
    * bounds; here we walk an in-memory buffer of many lines).
    */
  ...
  /** QUERY B (projection + filter, lazy string decode for survivors only): parseJson[Rec].filter(_.amount > t).map(_.category).toList Reads `amount`
    * (predicate) and `category` (projection, decoded LAZILY only for survivors); skips the other 18. Allocates a String only for the ~selectivity fraction of
    * records.
    */
  def categoryWhereAmountGt(buf: Array[Byte], from: Int, until: Int, threshold: Double): scala.collection.mutable.ArrayBuffer[String] =
    ...
    while lineStart < until do
      ...
      if amountSeen && amount > threshold && catSeen then out += decodeLatin1(buf, catStart, catLen)
      lineStart = lineEnd + 1
    out

  /** QUERY C — the user's insight: a PROJECTED numeric field doesn't need to become a `double` at all. `parseJson[Rec].filter(_.amount > t).map(_.id)` where
    * the result is, say, re-serialized or compared — `id` is captured as a raw byte SLICE `(start,len)` into the original buffer, exactly like a string. No
    * double/long decode, no allocation. The predicate field `amount` still decodes (you must compare it), but the projected `id` stays a slice. Here we just
    * return the survivors' id slices as packed longs (start<<32 | len) to prove zero decode — a real pipeline would carry these as an FArray column.
    *
    * This is the general principle: a number is decoded ONLY when an operation forces its numeric value (sum/compare/arithmetic). Pure projection/pass-through
    * keeps it a slice.
    */
  def idSlicesWhereAmountGt(buf: Array[Byte], from: Int, until: Int, threshold: Double): scala.collection.mutable.ArrayBuffer[Long] =
    ...
    while lineStart < until do
      ...
      if amountSeen && amount > threshold && idSeen then out += (idStart.toLong << 32) | (idLen.toLong & 0xffffffffL) // the raw slice, zero decode
      lineStart = lineEnd + 1
    out
