...
/** The decomposed-source contract the fusion engine drives — a **record-framed byte cursor**. Engine-neutral: any byte source that can hand back complete
  * records (NDJSON today, columnar/protobuf/etc. in future) implements it, and the engine lowers it to a per-record projection scanner via the
  * decomposed-source bridge (see `farray.RecordDecoder`). The engine asks for one thing: "hand me the byte range of the next COMPLETE record, in constant
  * memory, and tell me when records run out." Everything column-specific — which keys are live, skip-vs- decode, predicate early-out — is the
  * engine's/decoder's job; the source owns only framing + I/O.
  *
  ...
  *
  * '''Lifetime / constant memory.''' `buf` may be a reused/recycled array; a `(recordStart, recordEnd)` range is valid only until the next `nextChunk()`. A
...
trait ByteRecordSource:
  ...
  /** Frame the next COMPLETE record within the current chunk: sets `recordStart`/`recordEnd` and returns `true`, or returns `false` when the chunk is drained
    * (the driver then tries `nextChunk()`). A record that straddles a chunk boundary is the framer's problem, NOT the engine's — the engine only ever sees
