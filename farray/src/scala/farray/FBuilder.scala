...
object FBuilder:
  ...
  extension [A](b: FBuilder[A])
    ...
    /** bulk-append every element of any `IterableOnce`. Generic iteration, so primitive elements BOX at the
      * `Iterator[A]` boundary (inherent — the same second-class path as [[asScala]]); prefer [[addAll]] on an
      * `FArray[A]` for the unboxed leaf-arraycopy path. */
