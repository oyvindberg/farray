 ...
 *
 *  `Agg.xxx(...)` is SYNTAX, not a value: it is a marker the `agg`/`aggTo` macro reads off the AST (so it can
 *  inline the aggregate's lambda into the fused loop — the no-boxing / column-DCE win). It is NOT a usable
 *  runtime value — you cannot bind it to a `val`, store it, return it from a non-inline `def`, or build a
 *  `List` of them and pass them dynamically. Every factory is `@compileTimeOnly`, so any such misuse is a clear
 *  COMPILE ERROR pointing here, rather than a silent no-op. `R` is the aggregate's result type. */
