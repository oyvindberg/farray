...
/** The engine-facing seam for lowering a decomposed [[ByteRecordSource]] (a source that produces a record's fields
  * on demand). The fusion engine detects such a source and calls `lower`/`planString` here, handing a
  * [[DecomposedInput]]; it never names a concrete decoder. This indirection is the one place that binds the
