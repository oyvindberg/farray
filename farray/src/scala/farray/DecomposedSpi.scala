...
/** The boundary between the fusion engine and a decomposed-source decoder (a byte/columnar source that produces a record's fields on demand), expressed only in
  * `q.reflect` terms and functions over them — so the decoder never names the engine's `Shape`/`Tup`/`Ctx`/`Stage`, and the engine never names the decoder's
...
private[farray] final class DecomposedInput[Q <: Quotes & Singleton](using val q: Q)(
    ...
    /** terminal readers whose parameter IS the record type (so they contribute live fields), pre-decomposed to `(param, body)` by the engine (e.g. a fold op's
      * element param, an `Agg.sum(_.x)` lambda) — the decoder just walks them.
      */
...
/** one `map`/`filter` stage flattened to a plain lambda + its role. `isPositiveFilter` marks a `filter` (not `filterNot`) — the only kind eligible for the
  * inline predicate-fail early-out.
  */
...
/** what the decoder hands the engine per record: one column read per record field (live or dead), in declaration order. The engine wraps each into its `Shape`
  * (memoizing String columns) and rebuilds the record only if needed.
  */
private[farray] final class RecordColumns[Q <: Quotes & Singleton](using val q: Q)(
    val columns: List[Column[Q]]
)

/** a single field's read, as a `Shape`-free CPS thunk: `read(k)` yields the field-value `Term` to `k` (so a lazy String slice binds on first read). `isString`
  * tells the engine to MEMOIZE (decode-once); a numeric is an already- bound var and a dead field is a constant default, neither of which needs memoizing.
  */
