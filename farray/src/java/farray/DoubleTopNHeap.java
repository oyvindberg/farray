...
/** Unboxed top-N heap keyed by a {@code double} (for `topNBy(_.doubleField)` / `largest`/`smallest` over Double).
 *  Keys live in a primitive {@code double[]}; the per-element comparison is a direct {@code <}/{@code >} — NO
 *  boxing per offered element (the generic {@link TopNHeap} boxes the key on every comparison). See that class
 *  for the min-heap "largest" scheme; {@code largest=false} reverses the comparison for "smallest n". */
public final class DoubleTopNHeap {
    ...
    private boolean worse(double a, double b) { return largest ? a < b : a > b; }

    public void offer(Object elem, double key) {
        if (size < cap) {
            elems[size] = elem; keys[size] = key; size++;
            int i = size - 1;
            while (i > 0) { int p = (i - 1) >>> 1; if (!worse(keys[i], keys[p])) break; swap(i, p); i = p; }
