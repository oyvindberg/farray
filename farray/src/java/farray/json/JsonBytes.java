...
/**
 * SWAR (SIMD-within-a-register) byte-level scanning primitives — the hot inner loops of the fused
 * JSON scanner, in Java for bytecode control. Lifts jsoniter-scala / borer's 8-byte-at-a-time
 * tricks: read a {@code long} of input, test all 8 bytes at once for a target byte with the classic
 * "has-a-zero-byte" bit trick, and {@code numberOfTrailingZeros >> 3} to find which byte matched.
 *
 * <p>These replace the scalar byte-at-a-time {@code scanStringEnd}/{@code skipString} that the v0a
 * stack profile showed dominating (skipWs 26%, scanStringEnd 12%). The whole-document skip of
 * unwanted fields is the bulk of a projection query's work, so this is the lever for the throughput
 * target.
 */
public final class JsonBytes {
  ...
  /**
   * classic SWAR "find a zero byte": returns a word with the high bit set in each lane that is
   * zero.
   */
  ...
  /**
   * Scan a string body to its closing UNESCAPED quote. {@code start} points just after the opening
   * quote. Returns the index of the closing quote. Handles escapes by falling to a scalar step
   * whenever a backslash appears in the 8-byte window (rare for typical data).
   */
   ...
   */
  private static int scanScalarUntilQuoteOrBackslashConsumed(
      byte[] buf, int windowStart, int bi, int end) {
    int pos = windowStart + bi; // at the backslash
    // pos is a backslash: skip it and the escaped char. A unicode escape is 6 chars, but +2 is safe
    // to
    // re-enter the loop, which re-finds the next structural byte; the 4 hex digits are ordinary
    // bytes
