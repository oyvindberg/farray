...
/**
 * Open-addressing hash map with an UNBOXED {@code int} key, for the fused `groupReduceBy`/`groupCount`/
 * `groupSum` terminals — written in Java for the same reason the FArray core and the JSON scanner are: the
 * per-element probe loop is a hot, branch-heavy primitive and we want no {@code Integer.valueOf} per element.
 *
 * <p>Values are stored either in a primitive {@code long[]} (for Int/Long/Double accumulators — Int widened,
 * Long direct, Double via {@code doubleToRawLongBits}) or an {@code Object[]} (reference accumulators), chosen
 * at construction. The macro picks the right pair of accessors.
 *
 * <p>Hot-path contract (how the macro uses it, so no closure/SAM is ever stored here — `reduce` stays inlined
 * at the call site):
 * <pre>
 ...
 * </pre>
 * The returned slot is valid until the next {@code probe} (growth happens inside {@code probe}, before it
 * returns), and the macro uses it immediately — so lazy growth-on-probe is safe.
 */
public final class IntKeyMap {
    ...
    /** find-or-insert {@code key}; returns its slot index. Grows+rehashes first if needed. Sets {@link #wasNew}. */
    public int probe(int key) {
        if (size >= growAt) grow();
        int i = mix(key) & mask;
        while (used[i]) {
            if (keys[i] == key) { wasNew = false; return i; }
            i = (i + 1) & mask;
        }
        used[i] = true;
        keys[i] = key;
    ...
    public boolean wasNew() { return wasNew; }

    public long getValuePrim(int slot) { return valuesPrim[slot]; }
    public void setValuePrim(int slot, long v) { valuesPrim[slot] = v; }
    public Object getValueRef(int slot) { return valuesRef[slot]; }
    public void setValueRef(int slot, Object v) { valuesRef[slot] = v; }

    public int size() { return size; }

    private void grow() {
        int[] ok = keys; boolean[] ou = used; long[] ovp = valuesPrim; Object[] ovr = valuesRef;
        alloc((mask + 1) << 1);
