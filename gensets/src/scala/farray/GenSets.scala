...
/** Generates the sealed FSet core hierarchy `SBase` (sealed Java, leaves `final`, NO node tag — §1.6.10) + the per-kind-specialized `FSetOps` (the shared
  * NON-inline leaf methods + the build/membership helpers) + the Java `SetTraversers` (the membership-oracle predicate SAM + the foreach consumer SAM).
  *
  * Mirrors `GenCores` verbatim where it can: the `Emit` helper (real indentation, one statement per line), the `Kind`/`Box` model, `summonFrom` kind dispatch
  * on the REUSED `farray.Repr[A]` (so there is no second `Repr`), the typed-`Object[]` Ref trick, and the "small shared non-inline leaf method, JIT-inlinable
  * AND shareable" discipline (§1.6.4).
  *
  * SCAFFOLD/M1 SCOPE (the green vertical slice): `SBase` + `SView`/`SMaterialized` intermediates + `Empty` + `${K}One` + `${K}Sorted` (sorted prim leaf) + the
  * lazy `Union`/`Inter`/`Diff` algebra nodes, plus the ops `empty`/`apply`/`fromArray`/`fromFArray`/`from`/`contains`/`size`/`isEmpty`/`incl`(`+`)/`excl`(`-`).
  * The remaining FSet surface (Hash/Dense leaves, the traversal/materialize ops, `&`/`diff`/`xor`/`map`/`filter`/ `iterator`) is intentionally NOT emitted yet
  * — see the NEXT-STEPS in the design.
  *
...
object GenSets extends BleepCodegenScript("GenSets") {
  ...
  private def smaterialized: String = {
    val permits =
      ("SEmpty" :: opKinds.map(k => s"S${k.name}One") ::: opKinds.map(k => s"S${k.name}Sorted") ::: opKinds.map(k => s"S${k.name}Hash") ::: List("SIntBitmap"))
        .mkString(", ")
    s"""package farray;
       |
       |// GENERATED by GenSets — do not edit.
  ...
  /** the single empty set — kind-agnostic, a shared instance (mirrors FArray's Empty). SMaterialized: it is trivially deduped/enumerable, so FSet.empty answers
    * size()/iterate with no materialize step.
    */
  ...
  /** the sorted-prim-array materialized leaf — the default leaf shape (§1.1). `arr` is SORTED and DEDUPED; `size` is exactly `arr.length` (we never
    * over-allocate the final array), so the JIT can bound a scan on `arr.length` and drop the bounds check (§1.6.5).
    */
  private def sortedNode(k: Kind): String =
    if k.isPrim then s"""package farray;
         |
  ...
  /** the frozen open-addressing HASH leaf — the O(1)-contains workhorse past the size threshold (§2.3). The compact form: a dense `arr` (sorted+distinct for
    * prims — so it doubles as the merge's sorted input — any order for Ref) + a sparse `index` of POSITIONS into `arr` (-1 = empty). Built once, frozen. Ref
    * adds a parallel `hashes` so a probe rejects a non-matching slot with one int compare before any .equals/deref.
    */
  ...
  /** the dense-Int bitmap leaf (Step 2 / Roaring "bitmap container"): a `long[]` covering [base, base+64·words), one bit per value. contains is an O(1)
    * bit-test (no hash, no probe walk) — the answer to BitSet on dense non-negative-span Int. `card` is the precomputed popcount (the exact size). Chosen by a
    * density router (span ≤ K·n and span ≤ cap); sparse/wide stays on Sorted/Hash.
    */
  ...
  /** a contiguous range predicate leaf (§2.5): contains = lo<=x<=hi, O(1), ~24 bytes for any range incl. a billion elements. SView (membership-only /
    * non-materializable here). above(k)=[k+1,MAX], below(k)= [MIN,k-1], universal=[MIN,MAX] all fall out of the bounds (canonicalized in the factory).
    */
  ...
  /** the complement predicate leaf: contains = !inner.contains. Co-finite (≈ infinite) over a bounded domain, so SView / membership-only. Kind-agnostic (inner
    * carries its kind).
    */
  ...
  /** a lazy 2-field algebra node — O(1) construct, shares both operands (§1.3). Kind-agnostic (the operands carry their own kind). `sizeHint` is a cheap upper
    * bound; exact size forces a materialize.
    */
  ...
  private def fsetOps: String = {
    ...
    val containsLeaves = {
      ...
      opKinds.foreach { k =>
        ...
        ee.line(
          s"case x: SXor => x.left match { case _: SView => { flip ^= containsLeaf$K(x.right, e); node = x.left }; case l => return (containsLeaf$K(l, e) ^ containsLeaf$K(x.right, e)) ^ flip }"
        )
        ...
        if k.isPrim then {
          ...
          ee.line("val hc = e.hashCode()")
          // cap-banded mix: tiny caps keep mixInt, the [2^10, 2^15] band keeps the cheap mixRef fold, and
          // LARGE caps (> 32768) use the cheap NONLINEAR mix — the xorshift kills the key-family correlation
          // a bare multiply preserves, the multiply spreads, slot comes from the HIGH bits (valid at any cap)
          // and fp from the product's well-mixed middle bits. MEASURED at @100k vs murmur: Hit +10%,
          // varied-probe +14%, Miss −2% (still fastest).
          ee.line("val cl = ctrl.length")
          ee.line("var slot = 0")
          ee.line("var fp: Byte = 0")
          ee.open("if (cl > 32768)")
          ee.line("val m = (hc ^ (hc >>> 16)) * 0x9e3779b1")
          ee.line("fp = (((m >>> 7) & 0x7f) | 0x80).toByte")
          ee.line("slot = m >>> (java.lang.Integer.numberOfLeadingZeros(cl) + 1)")
          ee.closeOpen("else")
          ee.line("val m = if (cl >= 1024) mixRef(hc) else mixInt(hc)")
          ee.line("fp = (((m >>> 24) & 0x7f) | 0x80).toByte")
          ee.line("slot = m & (cl - 1)")
          ee.close()
          ee.line("var res = false; var go = true")
          ee.open("while (go)")
        ...
        if k.name == "Int" then
          ee.line("case b: SIntBitmap => { val i = e - b.base; return (i >= 0 && (i >>> 6) < b.words.length && (b.words(i >>> 6) >>> i & 1L) != 0L) ^ flip }")
    ...
    val buildLeaves = {
      ...
      opKinds.foreach { k =>
        ...
        if k.isPrim then {
          ...
          ee.line("val largeCap = cap > 32768") // probe agrees on the band via ctrl.length
          ee.line("val capShift = java.lang.Integer.numberOfLeadingZeros(cap) + 1")
          ee.line("var w = 0")
          ee.line("var i = 0")
          ee.open("while (i < len)")
          ee.line("val v = raw(i)")
          ee.line("val hc = v.hashCode()")
          ee.line("var fp: Byte = 0; var slot = 0")
          ee.open("if (largeCap)")
          ee.line("val m = (hc ^ (hc >>> 16)) * 0x9e3779b1")
          ee.line("fp = (((m >>> 7) & 0x7f) | 0x80).toByte")
          ee.line("slot = m >>> capShift")
          ee.closeOpen("else")
          ee.line("val m = if (cap >= 1024) mixRef(hc) else mixInt(hc)")
          ee.line("fp = (((m >>> 24) & 0x7f) | 0x80).toByte")
          ee.line("slot = m & (cap - 1)")
          ee.close()
    ...
    val mergeCore = {
      ...
      prims.foreach { k =>
        ...
        ee.lines(s"""def isUnsetAlg$K(node: SBase): Boolean = node match {
      ...
      // ---- Int dense-bitmap helpers (Step 2): density router + bitmap build + bit-iteration extraction.
      {
        ...
        ee.lines("""def finishBitmapInt(base: Int, card: Int, words: Array[Long]): SBase =
        ...
        ee.lines("""def bitmapMergeInt(lb: SIntBitmap, rb: SIntBitmap, op: Int): SBase = {
      ...
      // the algebra is CLOSED over the leaf — chained materialized Ref algebra stays unboxed.
      {
        ...
        refMerge(
          "mergeUnionRef",
          "na + nb",
          "outA(w) = aA(i); outH(w) = ha; w += 1; i += 1",
          "outA(w) = bA(j); outH(w) = hb; w += 1; j += 1",
          tieAllA = true,
          tieKeepIfFound = true,
          emitBNotInA = true,
          tailA = true,
        ...
        ee.lines("""def isUnsetAlgRef(node: SBase): Boolean = node match {
            |  case u: SUnion => u.memo == null
            |  case n: SInter => n.memo == null
            |  case d: SDiff => d.memo == null
            |  case x: SXor => x.memo == null
    ...
    val fromArrayV = dispatchA { k =>
      if k.isPrim then s"{ val src = as.asInstanceOf[Array[${k.arr}]]; buildSorted${k.name}(src, src.length, false) }"
      else s"{ val src = as.asInstanceOf[Array[Object]]; buildSorted${k.name}(src, src.length, false) }"
    }
    ...
    val inclV = dispatchB(k => s"{ val e = ${unwrapElemB(k, "elem")}; if (containsLeaf${k.name}(xs, e)) xs else new SUnion(xs, new S${k.name}One(e)) }")
    // excl: Diff(this, One(elem)) — O(1); returns `this` if absent.
    val exclV = dispatchB(k => s"{ val e = ${unwrapElemB(k, "elem")}; if (!containsLeaf${k.name}(xs, e)) xs else new SDiff(xs, new S${k.name}One(e)) }")
    ...
    val sameElementsV = dispatchA(k => if k.isPrim then s"setEq${k.name}(a, b)" else "setEqRef(a, b)")
    ...
    val minV = dispatchA(k =>
      if k.isPrim then
        s"""{ val arr = asArr${k.name}(materialize${k.name}(xs)); if (arr.length == 0) throw new NoSuchElementException("min of empty FSet") else (arr(0)).asInstanceOf[A] }"""
      else """throw new UnsupportedOperationException("min requires an ordered set (Ordering / FSortedSet)")"""
    )
    val maxV = dispatchA(k =>
      if k.isPrim then
        s"""{ val arr = asArr${k.name}(materialize${k.name}(xs)); if (arr.length == 0) throw new NoSuchElementException("max of empty FSet") else (arr(arr.length - 1)).asInstanceOf[A] }"""
