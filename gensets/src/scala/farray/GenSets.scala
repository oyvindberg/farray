  ...
  private def hashNode(k: Kind): String =
    if k.isPrim then
      ...
      s"""package farray;
         ...
         |// element's hashCode (high 32) and its position+1 in `arr` (low 32; 0 = empty) into ONE long, so a probe
         |// rejects a miss with a SINGLE cache-linear load — no random hashes[p] indirection (Swiss-table-lite).
         |// `hashes` (parallel to the hash-sorted `arr`) is kept for the unboxed merge core. O(1) contains.
         |public final class S${k.name}Hash extends SMaterialized {
         |    public final Object[] arr;
         |    public final int[] hashes;
         |    public final long[] slots;
         |    public S${k.name}Hash(Object[] arr, int[] hashes, long[] slots) { this.arr = arr; this.hashes = hashes; this.slots = slots; }
         |    @Override public int size() { return arr.length; }
         |}
         |""".stripMargin
  ...
  private def fsetOps: String = {
    ...
    val containsLeaves = {
      ...
      opKinds.foreach { k =>
        ...
        if k.isPrim then {
          ...
          // packed slots: ONE cache-linear load per probe; a miss rejects with no hashes[p]/arr[p] deref.
          ee.line("val slots = h.slots")
          ee.line("val hc = e.hashCode()")
          ee.line("var slot = mixInt(hc) & (slots.length - 1)")
          ee.line("var res = false; var go = true")
          ee.open("while (go)")
          ee.line("val s = slots(slot)")
          ee.line("if (s == 0L) go = false")
          ee.line("else if ((s >>> 32).toInt == hc && arr((s & 0xffffffffL).toInt - 1).equals(e)) { res = true; go = false }")
          ee.line("else slot = (slot + 1) & (slots.length - 1)")
          ee.close()
          ee.line("return res")
        }
        ee.close()
    ...
    val mergeCore = {
      ...
      // the algebra is CLOSED over the leaf — chained materialized Ref algebra stays unboxed.
      {
        ...
        ee.line("var cap = 8")
        ee.line("while (cap < n * 2) cap <<= 1")
        ee.line("val slots = new Array[Long](cap)")
        ee.line("var p = 0")
        ee.open("while (p < n)")
        ee.line("val hc = hashes(p)")
        ee.line("var slot = mixInt(hc) & (cap - 1)")
        ee.line("while (slots(slot) != 0L) slot = (slot + 1) & (cap - 1)")
        ee.line("slots(slot) = (hc.toLong << 32) | ((p + 1).toLong & 0xffffffffL)")
        ee.line("p += 1")
        ee.close()
        ee.line("new SRefHash(arr, hashes, slots)")
        ee.close()
        ee.close()
