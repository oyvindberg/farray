...
object GenSets extends BleepCodegenScript("GenSets") {
  ...
  private def sview: String =
    s"""package farray;
       ...
       |    permits SUnion, SInter, SDiff, SXor {
       |}
       |""".stripMargin
  ...
  private def fsetOps: String = {
    ...
    val containsLeaves = {
      ...
      opKinds.foreach { k =>
        ...
        if k.isPrim then {
          ...
          // Ref leaf: linear equals scan (no Ordering threaded yet — a RefHash leaf is the v1 NEXT-STEP).
          ee.line("val rn = a.length")
          ee.line("var i = 0")
          ee.line("var found = false")
          ee.open("while (i < rn && !found)")
          ee.line("if (a(i) == e) found = true")
          ee.line("i += 1")
          ee.close()
          ee.line("return found")
        }
    ...
    val mergeCore = {
      ...
      prims.foreach { k =>
        ...
        ee.line("case _ => SEmpty.INSTANCE")
        ee.close()
      }
      ee.result
    }
    ...
    val iteratorV = dispatchA(k =>
      if k.isPrim then s"asArr${k.name}(materialize${k.name}(xs)).iterator.asInstanceOf[Iterator[A]]"
      else "{ val acc = scala.collection.mutable.HashSet.empty[Object]; collectElemsRef(xs, acc); acc.iterator.asInstanceOf[Iterator[A]] }"
    )
    ...
    val sizeHelpers = {
      ...
      opKinds.foreach { k =>
        ...
        if k.isPrim then
          // a lazy algebra node: materialize (unboxed sorted merge) to a leaf, read its O(1) size.
          ee.line(s"case _ => materialize$K(node).asInstanceOf[SMaterialized].size()")
        else {
          ee.open("case _ =>")
          ee.line(s"val acc = scala.collection.mutable.HashSet.empty[${k.arr}]")
    ...
    val valueHelpers = {
      ...
      opKinds.foreach { k =>
        ...
        if k.isPrim then {
          ...
          ee.line(s"collectElems$K(node, acc)")
          ee.line("var h = 0")
          ee.line("acc.foreach(e => h += mixInt(e.hashCode()))")
          ee.line("h + acc.size")
          ee.close()
        }
