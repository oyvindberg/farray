...
/** COMPILE-ONLY gate (project `nulls-tests`, built with `-Yexplicit-nulls`). Every def below expands a
  * macro (`.fuse` pipelines) or an inline op (`distinctBy`, …) at THIS call site; under explicit nulls a
  * bare `null` flowing into a non-nullable type in the expansion is a hard type error. If this project
  * compiles, the generated surface is explicit-nulls-clean and `.fuse` / `distinctBy` are adoptable inside
  * a codebase compiled with `-Yexplicit-nulls` (e.g. the Scala 3 compiler itself). */
object NullsCompile:
