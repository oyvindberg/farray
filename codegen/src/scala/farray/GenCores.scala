...
object GenCores extends BleepCodegenScript("GenCores") {
  ...
  private def dfsDef(k: Kind): String = {
    ...
    s"""  inline def dfs${k.name}(root: FBase)(inline onLeaf: (Array[${k.arr}], Int) => Unit)(inline onOne: ${k.arr} => Unit): Unit = {
       ...
       |        case pad: ${k.name}Pad => { val pn = pad.length; var pi = 0; while (pi < pn) { onOne(${k.lc}At(pad, pi)); pi += 1 } }$rngCase
       |        case _ => ()
       |      }
       |    }
       |  }""".stripMargin
  ...
  private def atDef(k: Kind): String = {
    ...
    s"""  def ${k.lc}At(node: FBase, i: Int): ${k.arr} = node match {
       ...
       |    case pad: ${k.name}Pad => if (i < pad.base.length) $padBase else pad.filler$rngCase
       |    case _ => ${k.dflt}
       |  }""".stripMargin
  }
  ...
  private def fbase: String = {
    ...
    val permits = (leaves ++ List("RefArr", "RefAppend", "RefPrepend", "Concat", "RangeNode", "ReverseNode") ++ padKinds.map(_._1 + "Pad")).mkString(", ")
    s"""package farray;
       |
       |// GENERATED by GenCores — do not edit.
