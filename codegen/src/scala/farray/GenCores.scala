...
object GenCores extends BleepCodegenScript("GenCores") {
  ...
  private def dfsDef(k: Kind): String = {
    ...
    s"""  inline def dfs${k.name}(root: FBase)(inline onLeaf: (Array[${k.arr}], Int) => Unit)(inline onOne: ${k.arr} => Unit): Unit = {
       ...
       |        case u: ${k.name}Updated => { val un = u.length; var ui = 0; while (ui < un) { onOne(${k.lc}At(u, ui)); ui += 1 } }$rngCase
       |        case _ => ()
       |      }
       |    }
       |  }""".stripMargin
  ...
  private def atDef(k: Kind): String = {
    ...
    s"""  def ${k.lc}At(node: FBase, i: Int): ${k.arr} = node match {
       ...
       |    case u: ${k.name}Updated => if (i == u.index) u.elem else ${if k.name == "Ref" then "u.base.applyBoxed(i)" else s"${k.lc}At(u.base, i)"}$rngCase
       |    case _ => ${k.dflt}
       |  }""".stripMargin
  }
  ...
  private def fbase: String = {
    ...
    val permits = (leaves ++ List("RefArr", "RefAppend", "RefPrepend", "Concat", "RangeNode", "ReverseNode") ++ padKinds.map(_._1 + "Pad") ++ padKinds.map(_._1 + "Updated")).mkString(", ")
    s"""package farray;
       |
       |// GENERATED by GenCores — do not edit.
