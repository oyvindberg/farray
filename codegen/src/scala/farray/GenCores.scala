...
object GenCores extends BleepCodegenScript("GenCores") {
  ...
  private def reprSrc: String = {
    ...
    s"""package farray
       ...
       |
       |/** Array-representation evidence for element type `A`: `Prim` is the unboxed storage element and
       |  * `unwrap`/`wrap` convert between `A` and `Prim`. `unwrap`/`wrap` are `inline` and ABSTRACT here so
       |  * that, at each kind-dispatched op's `summonFrom { case r: $${K}Repr[A] => r.unwrap(…) }`, the
       |  * concrete given's identity conversion beta-reduces away and the primitive stays unboxed.
       |  *
       |  * ==Forwarding a `$${K}Repr` for an opaque-over-primitive type (recommended idiom)==
       |  * farray's kind dispatch does not see through opaque types (by design), so an
       |  * `opaque type FlagSet = Long` needs its element kind forwarded EXPLICITLY, or `FArray[FlagSet]`
       |  * would build `long[]` where `FlagSet` is transparent but box where it is opaque. Put the given in
       |  * the opaque's COMPANION (the implicit scope of an opaque type only includes an object of the same
       |  * name in the same scope), at the ELEMENT type, using the `given … with` shape so `unwrap`/`wrap`
       |  * have CONCRETE inline bodies:
       |  * {{{
       |  *   opaque type FlagSet = Long
       |  *   object FlagSet:
       |  *     given repr: farray.LongRepr[FlagSet] with
       |  *       inline def unwrap(a: FlagSet): Long = a
       |  *       inline def wrap(p: Long): FlagSet = p
       |  * }}}
       |  * Two shapes to AVOID:
       |  *  - `given farray.LongRepr[FArray[FlagSet]] = …` — the WRONG type argument (the array, not the
       |  *    element). This is the only shape that makes the synthesized given's parent `LongRepr[FArray[…]]`,
       |  *    which has tripped a dotty `computeBaseData` "non-class parent" assertion when the defining code
       |  *    is later read from TASTy as a library. Always forward at the ELEMENT type (`LongRepr[FlagSet]`).
       |  *  - `given farray.LongRepr[FlagSet] = someCanonicalInstance.asInstanceOf[…]` — a `given val` whose
       |  *    static type is the trait cannot reduce the abstract inline `unwrap`/`wrap` ("Deferred inline
       |  *    method … cannot be invoked"); the `given … with` shape above is required.
       |  *  A sub-typed opaque (`opaque type Flag <: FlagSet = Long`) needs its OWN given: `$${K}Repr` is
       |  *  invariant, so `LongRepr[FlagSet]` does not cover `Flag`.
       |  */
       |sealed trait Repr[A] {
       |  type Prim
  ...
  private def farrayOps: String = {
      ...
      def setup(kb: Kind): String = kb.name match {
        ...
        case "Int"            =>
          ...
          "var cap = 16; while (cap.toLong < (n.toLong << 1) && cap != (1 << 30)) cap <<= 1; " +
            ...
            // -Yexplicit-nulls). The `bmSpan` floor is main's tuned small value (see the audit note above).
    ...
    // mapNotNone: apply f: A => Option[B] ONCE per element, keep the Some contents, drop the Nones — FUSED in
    // ONE pass. Same double-kind-dispatch shape as filterMap/collect (A read x B write over the shared
    // foreachLeaf${I} traverser into an unboxed ${O}Group buffer); f is INLINE so it fully inlines and is
    // evaluated EXACTLY ONCE per element (result stored in `o`, tested with isDefined, unwrapped with get). No
    // intermediate FArray. The Option is allocated by the caller's lambda (unavoidable); the A/B handoff boxes
    // (same NOTED limitation as filterMap); the OUTPUT stays unboxed via the ${O}Group.
    val mapNotNone = withErr(
      "summonFrom {\n" + opKinds
        .map { ka =>
          val inner = "summonFrom {\n" + opKinds
            .map { kb =>
              val add = s"buf.add(${wr(kb, "rb.unwrap(o.get)")})"
              s"          case rb: ${kb.name}Repr[B] => { val buf = new ${kb.name}Group(); foreachLeaf${ka.name}(xs, (v) => { val a = ${wrapV(ka)}; val o = f(a); if (o.isDefined) $add }); buf.toLeaf }"
            }
            .mkString("\n") + "\n        }"
          s"      case r: ${ka.name}Repr[A] => $inner"
        }
    ...
    s"""package farray
       ...
       |  inline def mapNotNoneImpl[A, B](xs: FBase)(inline f: A => Option[B]): FBase = $mapNotNone
