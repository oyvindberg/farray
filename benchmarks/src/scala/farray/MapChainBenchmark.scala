...
class MapChainIntBenchmark extends IntInputs {
  ...
  @Benchmark def farray9(): Int =
    farrayInput.map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).foldLeft(0)(_ + _)
  @Benchmark def array9(): Int =
    arrayInput.map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).foldLeft(0)(_ + _)

  @Benchmark def farray12(): Int = farrayInput
    .map(_ + 1)
    .map(_ + 1)
    .map(_ + 1)
    .map(_ + 1)
    ...
    .map(_ + 1)
    .foldLeft(0)(_ + _)
  @Benchmark def array12(): Int = arrayInput
    .map(_ + 1)
    .map(_ + 1)
    .map(_ + 1)
    .map(_ + 1)
