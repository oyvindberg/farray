...
class SlicingIntBenchmark extends IntInputs {
  ...
  @Benchmark def kyochunk_partition(): (kyo.Chunk[Int], kyo.Chunk[Int]) = kyoChunkInput.partition(_ % 2 == 0)
  ...
  @Benchmark def kyochunk_span(): (kyo.Chunk[Int], kyo.Chunk[Int]) = kyoChunkInput.span(_ < size / 2)
  ...
  @Benchmark def kyochunk_splitAt(): (kyo.Chunk[Int], kyo.Chunk[Int]) = kyoChunkInput.splitAt(size / 2)
  ...
  @Benchmark def kyochunk_grouped(): Int = { var n = 0; kyoChunkInput.grouped(math.max(1, size / 2)).foreach(_ => n += 1); n }
  ...
  @Benchmark def kyochunk_sliding(): Int = { var n = 0; kyoChunkInput.sliding(2, 3).foreach(_ => n += 1); n }
}
