...
class UnzipIntBenchmark extends CommonParams {
  ...
  var kyoChunkInput: kyo.Chunk[(Int, Int)] = _
  @Setup def setup(): Unit =
    ...
    kyoChunkInput = kyo.Chunk.from(List.tabulate(size)(i => (i, i * 2)))
  ...
  @Benchmark def kyochunk(): (kyo.Chunk[Int], kyo.Chunk[Int]) = kyoChunkInput.unzip
...
class UnzipStrBenchmark extends CommonParams {
  ...
  var kyoChunkInput: kyo.Chunk[(String, Int)] = _
  @Setup def setup(): Unit =
    ...
    kyoChunkInput = kyo.Chunk.from(List.tabulate(size)(i => (i.toString, i)))
  ...
  @Benchmark def kyochunk(): (kyo.Chunk[String], kyo.Chunk[Int]) = kyoChunkInput.unzip
  // fs2.Chunk has no unzip
}
