...
object Publish extends BleepScript("Publish") {
  ...
  def run(started: Started, commands: Commands, args: List[String]): Unit = {
    ...
    val info = Info(
      "Immutable `Array`-based sequence replacement for Scala 3",
      "https://github.com/oyvindberg/farray/",
      List(
        Info.Developer(
          "oyvindberg",
          "Øyvind Raddum Berg",
    ...
    files.foreach { case (path, bytes) =>
      started.logger.withContext("path", path.asString).withContext("size", bytes.length.toString).debug("will publish")
    }
