  ...
  return (
    <section className="chapter" id="inside-map">
      ...
      <p>
        ...
        case is the fast path; a tree input falls through the <code>case _</code> to the shared traverser —
        the one direction-aware walk every op reuses. Now watch what happens at a concrete{" "}
        <code>FArray[Int]</code> call site. The compiler knows <code>A = Int</code>, so{" "}
      ...
      <p>
        ...
        <code>mapLeafIntInt</code> is deliberately not, so the one heavy loop is compiled a single time and
        shared. The price of crossing that hand-off is that the lambda has to become a function value — hence
        the SAM. But it's a <em>specialized</em> SAM, so nothing boxes, and it's one allocation per{" "}
      ...
      <p>
        ...
        <code>.fuse</code> — which is precisely why fusion is a separate gear. What the inline layer buys on
        its own is narrower but still decisive. Compare a generic <code>map[B](f: A =&gt; B)</code>, where{" "}
        <code>A</code> and <code>B</code> erase to <code>Object</code>, <code>f</code> is a boxed{" "}
        <code>Function1</code>, and every element round-trips through <code>Integer</code>. FArray never
      ...
      <p>
        And the point of all that machinery, measured — <code>map</code> over a hundred thousand elements, Int
        beside String. This is also where to be honest, because on its own <code>map</code> is no rout:
      </p>
      ...
      <p>
        ...
        the benchmark stops being a micro-benchmark — and it doesn't take much.
      </p>
      ...
      <p>
        ...
        sees eight different <code>Function1</code> classes — well past the JIT's ≥3-type megamorphic
        threshold, so it stops devirtualising: every element pays a real virtual <code>apply</code>, and on{" "}
        <code>Int</code> it re-boxes through <code>Integer</code> on the way out. The bigger the method, the
        more distinct lambdas crowd that one site, the worse it gets. FArray has no shared site to poison —
        each lambda was already bound to its own specialized loop at compile time, so eight maps are just
        eight tight <code>int[]</code> loops, indifferent to how many of them you wrote.
      </p>
      ...
      <p>
        And because all of that specialization happened at <em>compile</em> time, there's nothing for the JIT
        to discover. FArray's speed is already in the bytecode — unboxed, monomorphic, allocation-free — so it
