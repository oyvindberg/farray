    ...
    <article className="page">
      ...
      <section className="ex">
        ...
        <p>
          ...
          the projection case where pushdown pays. Throughput is ops/s over the whole 10 000-record buffer;
          allocation is bytes per buffer-op (÷10 000 ≈ bytes per record). Every parser runs the same query;
          the fuse row is the generated scanner, the rest each library's best idiom for "read a couple of
          fields per line."
        </p>
      </section>
      ...
      <section className="ex">
        ...
        <p>
          ...
          or allocating. A 20-field record costs one number-parse and 19 byte-skips. No <code>Event</code> is
          built; the fold runs into a <code>var</code>.
        </p>
        <JsonEx
      ...
      <section className="ex">
        ...
        <p>
          <code>category</code> gets a slot, but a string slot is just <code>(start, len)</code> into the buffer
          — not a decoded String. <code>amount</code> decodes (the filter needs it); <code>category</code> stays
          a byte slice and becomes a real <code>String</code> only inside the survivor <code>if</code>. Rejected
          rows never allocate a String — the same compute-for-survivors sink from the optimizer page, now over
          bytes.
        </p>
        <JsonEx
      ...
      <section className="ex">
        ...
        <p>
          <code>count</code> ignores the element, so <code>map(_.category)</code> is a dead value. The optimizer
          captures the slice but never turns it into a String — zero String allocations. Same for{" "}
          <code>isEmpty</code>/<code>nonEmpty</code>, and for <code>map(expensive).count</code>, where{" "}
          <code>expensive</code> never runs. Dead-column elimination reaching the terminal.
        </p>
      ...
      <section className="ex">
        ...
        <p>
          The filter field is decoded <em>during</em> the scan; the instant it's known, the predicate is checked
          inline, and on failure the record is abandoned right there — the scanner never looks for the later
          projected field. Here <code>key</code> is first, <code>payload</code> (a long string) is last, and 90%
          of records are rejected: the rejected ones cost only "scan to <code>key</code>, compare, stop." This
          is the biggest win on fat selective queries.
        </p>
      ...
      <section className="ex">
        ...
        <p>
          Sum the amount, count the rows, and take the peak score of the active events — <code>aggTo</code> runs
          all three in a single fused scan, carrying one (unboxed) accumulator each. The columns the aggregates
          read are merged automatically: <code>amount</code>, <code>status</code>, and <code>score</code> are
          scanned (3 of 20 fields), each once; <code>count</code> reads nothing extra. It returns a case class,
          not a tuple; no <code>Event</code> is built, and the sum/max use <code>dadd</code>/<code>dcmp</code>,
          not a boxing <code>Numeric</code>/<code>Ordering</code>.
        </p>
