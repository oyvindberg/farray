...
<p className="lede">
  ...
  leaf node. That gap is accepted: the design principle says losing to a bare array on
...
problem it avoids. A normal `apply(as: A*)` receives its arguments as a `Seq[A]`: the
