...
<p className="lede">
  ...
  <code>IArray(1, 2, 3)</code> (IArray is Scala's immutable view of a raw array) is one array
  ...
  wrapper libraries (Scala's <code>List</code>, an immutable linked list, and <code>Vector</code>,
  a persistent tree, plus the array-backed <code>Chunk</code> types from the fs2 and ZIO streaming
