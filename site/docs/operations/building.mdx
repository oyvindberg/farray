...
<BenchPair
  int="ConstructIntBenchmark"
  str="ConstructStrBenchmark"
  caption="fill and tabulate, building `size` elements from a rule, Int beside String. `fill` on Int ties IArray and leaves the boxing collections behind (List 19–25×, Vector 6.6–7.9×). `tabulate` on Int is the standout: FArray pulls ~13.8× ahead of IArray at 100k, because `FArray.tabulate` takes its function `inline` and fills the `int[]` directly, while `Array.tabulate` is generic and boxes every `f(i)` through a `Function1` — the allocation profiler measures 2.0 MB/op for IArray against FArray's 0.40 MB (one `int[]`, nothing extra). On String every element allocates anyway, so the gap narrows to structure alone (1.5–2.2× over the field)."
/>
