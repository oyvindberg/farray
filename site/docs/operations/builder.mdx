...
(Scala's `inline` is a compiler-guaranteed expansion at the call site, not a JIT hint), pointed at an
...
Two builders can't meaningfully pre-size (`Vector.newBuilder` and `List.newBuilder`, for Scala's
persistent-tree Vector and immutable linked List, have `sizeHint`
