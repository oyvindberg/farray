...
There's no new machinery here — it's exactly the inline, compile-time kind dispatch from [map](/operations/map), pointed at an
append instead of a transform. `FBuilder[A]`
is an opaque type over a generated `FBuilderBase` whose concrete subclasses are the per-kind
