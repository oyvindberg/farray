...
<p className="page__eyebrow">concept four of five</p>

<p className="lede">
  Fusing stages into one loop removes the plumbing tax. This page is about the second engine inside
  the engine — the part that removes work you <em>wrote but never needed</em>. Its single organizing
  idea: a product — a tuple, a case class, later a JSON record — is not a value, it's a set of
  independent <strong>columns</strong>, and each column is computed at its first read or not at all.
  Everything below falls out of that idea; every emitted block is a golden test.
</p>
