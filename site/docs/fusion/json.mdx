...
description: "A JSON decoder plugged into the fusion engine as a typeclass instance — beating the JVM's best hand-tuned parser on projection queries, with the whole integration in one readable file."
---
...
<p className="lede">
  ...
  Here is the entire program — a case class and a query. No codec, no <code>derives</code>, no
  registration; the case class <em>is</em> the schema:
</p>
...
And here is what that does to the field — including jsoniter-scala driven by a hand-written,
zero-object `JsonReader` loop, which is as fast as JSON gets on the JVM by conventional means:
...
one question: **is the fusion engine about collections, or about pipelines?** So the engine was
pointed, unchanged, at raw bytes. Read it as a measure of how far the machinery reaches — and as the
...
verbatim, at the byte level: the macro computes the *live set* — the fields your filters, maps and
aggregates actually touch — and emits a per-record scanner where every live field gets a typed byte
...
The queries on this page run against a 10 000-record, ~3.2 MB NDJSON buffer of that 20-field `Event`
— realistic field mix, every query touching 1–2 fields: the projection case where pushdown pays.
...
Before the demos, the part a library author should read closely. This is the **entire** plug-in —
the shape, the typeclass instance, the hook. Not a sketch; the file, verbatim:
...
shape; the given in its companion puts `JsonLowering`'s terminals on every `Fuse[A, Ndjson]`;
`StandardTerminals` declares which families this shape supports (everything except element
materialization — so `.sum` scans bytes while `.run` *does not compile*); and the five-line hook
hands the engine `JsonDecode` — the decoder — as an ordinary argument at macro-expansion time.
...
The **decoder** (`JsonDecode.scala`) is the format's genuine work — given one framed record and the
live set, emit the byte-level readers. The engine keeps everything column-aware: live-set analysis,
skip-versus-decode, predicate placement, the loop itself. That split of labor is why the decoder
stays small, and why a CSV or length-prefixed-binary decoder would be the same shape: implement the
framing, implement the per-field reads, publish a given.

Three ways in, one per place your bytes live — the file and stream variants run in constant memory
(one block + the largest record), with the source closed on every exit path:
...
`String` never built — zero String allocations across the run. Dead-column elimination reaching all
...
The filter field is decoded *during* the scan, the predicate runs the instant it's known, and a
failing record is abandoned right there — the scanner never even looks for the later projected
...
call site, and an unsupported *terminal* is simply not a member. No silent fallback exists.
