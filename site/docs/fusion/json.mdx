...
The plug-in itself (shape, lowering, source, decoder, and the contract at each seam) is dissected
on <Link to="/fusion/integrating">Integrating with fusion</Link>; it is one readable file. What
this page adds is the proof it works: real queries, the scanners the macro emitted for them, and
the rivals.
