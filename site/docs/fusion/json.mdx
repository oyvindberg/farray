...
And here is how that measures against the field, including jsoniter-scala driven by a hand-written,
...
shape; the given (Scala's keyword for publishing such a compiler-resolved instance) in its
companion puts `JsonLowering`'s terminals on every `Fuse[A, Ndjson]`;
...
The **decoder** (`JsonDecode.scala`) is the format-specific work: given one framed record and the
