...
/** A long fused chain in ONE method. The whole pipeline inlines into `longChain`'s body; for the JVM JIT to
 *  compile it, that body must stay under the HugeMethodLimit (~8000 bytecodes, -XX:-DontCompileHugeMethods).
 *  The fused macro emits one loop nest with small inlined stage bodies (the heavy traversal is the plain
 *  indexed while-loop, never a duplicated dfs), so this should stay well under the limit. Measured by javap
 *  in the no-blowup test below. */
object FuseSize:
