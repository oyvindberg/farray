...
/** Algebraic stage rewrites: adjacent `take(n).take(m)` → `take(min(n,m))` and `drop(n).drop(m)` → `drop(n+m)`, which collapse redundant per-element counter
  * machinery. Asserts both correctness (vs List) and that the collapse actually happened (one limit/counter in the generated code, not two).
  */
class RewriteTest:
  ...
  @Test def takeMapTake_value(): Unit =
    ...
    assertEquals(ls.take(6).map(_ * 3).map(_ - 1).take(2), xs.fuse.take(6).map(_ * 3).map(_ - 1).take(2).run.toList) // through two maps
  ...
  /** the commute means `map(f).take(n)` evaluates `f` at most n times (it already did via the done-flag, but the rewrite must not regress that). A call-count
    * proves f runs exactly n times over a long input.
    */
