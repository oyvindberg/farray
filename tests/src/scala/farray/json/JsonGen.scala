...
/** Deterministic NDJSON generator for the 20-field `Rec`, used by both the correctness test and the benchmark so they agree on the data. Field WRITE ORDER is
  * shuffled per record (seeded) so the scanner's any-order dispatch + skip logic is genuinely exercised, and `amount` is sometimes early / sometimes late in
  * the record (so predicate early-out is tested both ways).
  */
object JsonGen:
...
/** Fat-numeric NDJSON: 16 double metrics + id, shuffled. Kept in sync with the benchmark's fat generator. (Makes concrete the observation that a record is
  * "basically all numbers" and reading one while skipping the rest as bytes is the lazy-decode win.)
  */
object FatGen:
  private def fields(i: Int): Array[(String, String)] =
