...
/** COMPILE-ONLY gate (project `werror-tests`, built with `-deprecation -Werror`). Each def below expands
  * an inline op body at THIS call site. If any inline body expands a deprecated construct — notably
  * `a -> b` (Predef.ArrowAssoc, deprecated under recent dotty) in `groupBy`/`groupMap` — the deprecation
  * surfaces here as a hard error, exactly as it did in the Scala 3 compiler (a `-Werror` codebase that
  * needed a `-Wconf` exclusion). If this project compiles clean, the inline surface is `-Werror`-safe. */
